        ...
        try {
            if (fs.existsSync(configPath)) {
                ...
                if (!this._isPositionVisible(config.x, config.y, config.width, config.height)) {
                    // Si position hors �cran, recentrer
                    config.x = undefined;
                    config.y = undefined;
                }
                return config;
    ...
    create() {
        this._window = new BrowserWindow({
            ...
            webPreferences: {
                ...
                sandbox: false,
                nativeWindowOpen: true, // <- indispensable pour que window.open cr�e un vrai BrowserWindow
            },
        });

        // Assigne le m�me preload (et m�mes flags) aux fen�tres ouvertes via window.open
